    ...
    db.get(`SELECT * FROM settings WHERE serverid = '${message.guild.id}' AND setting = 'prefix'`, (err, row) => {
        ...
        if (args[0]) {
            ...
            embed.setDescription([
                ...
                `❯ **Category:** ${command.category ? command.category : "Normal" || "Admin"}`,
            ].join("\n"));
        ...
        categories.forEach(category => {
            ...
            try {
                ...
                if (message.author.id == "281348487340359681") embed.addField(`❯ ${capitalise}`, dir.map(c => `\`${c.help.name}\``).join(", "));
